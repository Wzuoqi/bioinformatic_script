...
def extract_data(data_block):
    # 提取项目编号
    project_number_match = re.search(r'^(\d+)', data_block)
    project_number = project_number_match.group(
        1) if project_number_match else ""

    # 提取项目名称
    project_name_match = re.search(r'^\d+\.\s(.+)$', data_block)
    project_name = project_name_match.group(
        1).strip() if project_name_match else ""
